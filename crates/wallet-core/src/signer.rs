...
/// Compute the SHA-256 transaction hash of the inner `TransactionV1Envelope` contained in
/// `inner_xdr`, using the given network's passphrase. Returns 32 raw bytes suitable for use
/// as a dedup key (e.g. `inner_tx_hash` in the sponsored-tx store).
pub fn compute_inner_tx_hash(
    ...
    let inner_env = TransactionEnvelope::from_xdr_base64(inner_xdr)
        .map_err(|_| WalletError::InvalidXdr)?;
    let tx = match inner_env {
        TransactionEnvelope::Transaction(tx) => tx,
        TransactionEnvelope::FeeBumpTransaction(_) => return Err(WalletError::InvalidXdr),
    };
    let hash_vec = tx.hash(&network.to_base()).map_err(|_| WalletError::Signing)?;
    hash_vec.try_into().map_err(|_| WalletError::Signing)
}
